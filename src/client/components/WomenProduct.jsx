...
function WomenProduct() {
  ...
  useEffect(() => {
    axios
      .get(`http://localhost:4000/product/category/${params.WomenWears}`)
      .then((res) => setWomenWear(res.data))
            ...
            {womenWear.map((prd) => (
              <div className="col mb-5" key={prd._id}>
                <div className="card h-100">
                  {/* <!-- Product image--> */}
                  <Link to={`/aboutproduct/${prd._id}/${params.WomenWears}`}>
                    {" "}
                    <img
